...
class DataLoaderLite:
    ...
    def next_batch(self):
        ...
        self.current_position += B*T  # we're not doing B * T + 1 here because in training the consumption was only till B * T, the +1 was used for target hence there is no overlap
        # if loading next batch would be out of bounds, reset
        if self.current_position + (B * T + 1) > len(self.tokens):
            self.current_position = 0  # 1 epoch completed (almost)
        return x, y
