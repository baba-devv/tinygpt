...
def write_datafile(filename, tokens_np):
    # writes a numpy array of uint16 tokens to a .npy file (np.save appends the extension),
    # so that dataset.load_tokens can read it back with np.load and recover the dtype
    np.save(filename, tokens_np)

# tokenize all documents and write output shards, each of shard_size tokens
def main():
    # stream the dataset so downloading overlaps tokenization instead of blocking on the full ~30GB
